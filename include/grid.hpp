...
namespace {
	...
	std::atomic<long> poly_fid(0);		// A feature ID for geometries.
	...
	namespace grid {
    	...
    	public:
    		...
    		GridProps() :
				...
				m_interleave(Interleave::Band) {	// The data type.
    		}

    		size_t index(int col, int row, int band) const {
    			if(band < 1 || band > m_bands)
    				g_runerr("Invalid band: " << band);
    			if(m_interleave == Interleave::Band) {
    				return (rows() * cols() * (band - 1)) + row * cols() + col;
    			} else {
    				return row * cols() * bands() + col * bands() + (band - 1);
    			}
    		}
            ...
            size_t sizeBands() const {
            	return m_cols * m_rows * m_bands;
            }
        ...
        public:
            ...
            void init(const std::string& filename, const GridProps& props) {
				...
				if(m_props.interleave() == Interleave::Band) {
             ...
             * \param buf A pre-allocated buffer to store the data.
             */
            int getColumn(int col, int band, T* buf);
             ...
             * \param buf A pre-allocated buffer to store the data.
             */
            int getPixel(int col, int row, T* buf);
             ...
             * \param band     The target band.
			 */
            void smooth(Grid<T>& smoothed, double sigma, int size, int band) {
            	smooth(smoothed, sigma, size, band, smoothCancel, smoothStatus);
            }
            ...
            void smooth(Grid<T>& smoothed, double sigma, int size, int band,
            	...
            	for(int i = 0; i < numThreads; ++i) {
            		threads.emplace_back(smooth, this, &smoothed, size, nodata, weights, &row,
            				&rmtx, &wmtx, &cancel, &status, &exceptions[i]);
            	}
            ...
            void polygonize(const std::string& filename, const std::string& layerName, const std::string& idField,
            		...
            		bool* cancel = nullptr, Status* status = nullptr) {
            ...
            void polygonize(const std::string& filename, const std::string& layerName, const std::string& idField,
            	...
            	// It's faster to work on a band-sequential raster.
            	if(props().interleave() == Interleave::Pixel)
            		remap(Interleave::Band);
            	...
            	for(int i = 0; i < threads; ++i)
            		ths.emplace_back(&polyWriteToFile, &geoms, &finalIds, &idField, layer, &gctx, removeHoles, removeDangles, &running, &cancel);
            	...
            	for(int r = 0; r < rows; ++r) {
            		...
            		status->update((float) r / rows, polyRowStatus(r, rows));
            ...
            void flush() {
            	...
            	if(m_ds && props().writable()) {
            		...
            		if(props().interleave() != Interleave::Band)
            			remap(Interleave::Band);
